...
class YHProgressView: UIView {
    ...
    lazy var proLabel:UILabel? = {
        let label = YHUtil.createLabelWith(Text: "", Frame: CGRect(x:0, y:0, width:self.bounds.size.width, height:self.bounds.size.height), TextColor: UIColor.white, Font: self.bounds.size.width/6, TextAligtment: .center)
